...
/**
 * Requête de validation pour la modification d'une réservation
 * 
 * Mêmes règles que StoreReservationRequest + validation du statut
 */
class UpdateReservationRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        if (!$this->user()->can('planning.edit')) {
            return false;
        }

        // Ne peut pas modifier une réservation terminée, annulée ou no-show
    ...
    public function rules(): array
    {
        return [
            ...
            // Statut (optionnel lors de la modification)
            'status' => [
                'sometimes',
                'string',
                Rule::enum(StatutReservationEnum::class),
                function ($attribute, $value, $fail) {
                    $reservation = $this->route('reservation');
                    $newStatus = StatutReservationEnum::from($value);
                    
                    // Vérifier les transitions de statut autorisées
                    $allowedTransitions = [
                        StatutReservationEnum::SCHEDULED->value => [
                            StatutReservationEnum::IN_PROGRESS->value,
                            StatutReservationEnum::CANCELLED->value,
                        ],
                        StatutReservationEnum::IN_PROGRESS->value => [
                            StatutReservationEnum::COMPLETED->value,
                            StatutReservationEnum::CANCELLED->value,
                        ],
                    ];
                    ...
                    if (isset($allowedTransitions[$currentStatus])) {
                        if (!in_array($value, $allowedTransitions[$currentStatus])) {
                            $fail("Transition de statut non autorisée de {$currentStatus} vers {$value}.");
                        }
                    } else {
                        $fail("Le statut actuel ne peut être modifié.");
                    }
                },
            ],
        ];
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Vérifier les conflits de disponibilité (en excluant la réservation actuelle)
    ...
    private function validateAvailability($validator): void
    {
        ...
        $conflicts = $planningService->detectConflits([
            ...
            'exclude_reservation_id' => $reservation->id, // Exclure la réservation actuelle
        ]);
